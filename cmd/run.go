...
func monitorKubernetesDeployment(appName string) {
	...
	type PodMetadata struct {
		Name              string  `json:"name"`
		DeletionTimestamp *string `json:"deletionTimestamp"` // Allows us to identify "ghost" pods
	}
	...
	for attempt := 0; attempt < maxAttempts && !finished; attempt++ {
		...
		for _, pod := range podList.Items {
			// THE FIX: If the pod is scheduled for deletion by Kubernetes, ignore it completely.
		...
		// Prevent declaring success if the only pods we found were being deleted
		if !activePodsFound {
			allRunningAndReady = false
		}
